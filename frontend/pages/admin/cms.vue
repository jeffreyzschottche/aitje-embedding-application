      ...
      <div v-else class="space-y-6">
        ...
        <section class="space-y-4">
          ...
          >
              ...
              <div>
                ...
                <div class="mt-3 flex flex-wrap gap-2 text-xs font-semibold">
                  ...
                  <span
                    ...
                    :class="user.git_configuration?.has_access_token ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-700'"
                  >
                    {{ user.git_configuration?.has_access_token ? 'Git token gekoppeld' : 'Git token ontbreekt' }}
                  </span>
                  <span class="rounded-full bg-loci-cream px-3 py-1 text-loci-gray-500">
                    Laatste sync: {{ user.git_configuration?.last_pushed_at ? formatDate(user.git_configuration.last_pushed_at) : 'nooit' }}
                  </span>
                </div>
              </div>
            ...
            <form class="mt-6 grid gap-4 md:grid-cols-2" @submit.prevent="saveGitConfig(user)">
              <div>
                <label class="mb-1 block text-sm font-semibold text-loci-black">Repository URL</label>
                <input v-model="user.gitForm.repo_url" required placeholder="https://github.com/bedrijf/repo.git" class="w-full rounded-loci border border-loci-gray-300 bg-loci-cream px-4 py-3 focus:border-loci-yellow focus:outline-none">
              </div>
              <div>
                <label class="mb-1 block text-sm font-semibold text-loci-black">Branch</label>
                <input v-model="user.gitForm.branch" required class="w-full rounded-loci border border-loci-gray-300 bg-loci-cream px-4 py-3 focus:border-loci-yellow focus:outline-none">
              </div>
              <div class="md:col-span-2">
                <label class="mb-1 block text-sm font-semibold text-loci-black">
                  Access token
                  <span v-if="user.git_configuration?.has_access_token" class="text-xs font-normal text-loci-gray-500">
                    (laat leeg om bestaande token te behouden)
                  </span>
                </label>
                <input v-model="user.gitForm.access_token" type="password" :required="!user.git_configuration?.has_access_token" class="w-full rounded-loci border border-loci-gray-300 bg-loci-cream px-4 py-3 focus:border-loci-yellow focus:outline-none">
              </div>
              <div class="md:col-span-2 flex items-center gap-3">
                <button
                  type="submit"
                  :disabled="user.savingGit"
                  class="rounded-loci-full bg-loci-yellow px-6 py-3 font-semibold text-loci-black-deep transition-all hover:bg-loci-yellow-hover disabled:bg-loci-yellow-light disabled:text-loci-gray-400"
                >
                  {{ user.savingGit ? 'Opslaan...' : 'Git-config opslaan' }}
                </button>
                <p v-if="user.gitMessage" class="text-sm text-green-700">{{ user.gitMessage }}</p>
                <p v-if="user.gitError" class="text-sm text-red-700">{{ user.gitError }}</p>
              </div>
            </form>
          </article>
        </section>
...
async function saveGitConfig(user: AdminUser) {
  user.savingGit = true;
  user.gitMessage = '';
  user.gitError = '';

  try {
    const response = await adminRequest<{ message: string; user: AdminUser }>(`/admin/users/${user.id}/git-config`, {
      method: 'POST',
      body: JSON.stringify(user.gitForm),
    });

    Object.assign(user, hydrateUser(response.user));
    user.gitMessage = response.message;
  } catch (error: any) {
    user.gitError = extractError(error) || 'Git-config opslaan mislukt';
  } finally {
    user.savingGit = false;
  }
}
...
function hydrateUser(user: AdminUser): AdminUser {
  return {
    ...user,
    gitForm: {
      repo_url: user.git_configuration?.repo_url || '',
      branch: user.git_configuration?.branch || 'main',
      access_token: '',
    },
    savingGit: false,
    gitMessage: '',
